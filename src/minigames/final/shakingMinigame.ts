import { FederatedMouseEvent, Assets, Point, Text, Sprite } from "pixi.js";
...
export class ShakingMinigame extends Minigame {
  ...
  override async attach() {
    ...
    const texture = await Assets.load(MINIGAME_ASSET_ALIASES.KITCHEN_BG);
  ...
  protected override async populateContainer() {
    const loadSprite = new Sprite(
      await Assets.load(MINIGAME_ASSET_ALIASES.BLENDERFILL),
    );
    const progSprite = new Sprite(
      await Assets.load(MINIGAME_ASSET_ALIASES.BLENDERPROG),
    );
    ...
    const aspectRatio = this.prog.width / this.prog.height;
    this.prog.width = this.app.screen.width * 0.25;
    this.prog.height = this.prog.width / aspectRatio;
    this.prog.x = this.prog.height / 2;
    this.prog.y = this.prog.height / 2;
  ...
  private onDrag(event: FederatedMouseEvent) {
    ...
    this.prog.progress =
      (this.score / (this.scoreToPass * this.amountOfBlenders)) * 100;
  }
}
