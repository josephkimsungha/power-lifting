import { Point, Ticker, Sprite, Assets } from "pixi.js";
...
export class BalancingMinigame extends KeyboardMinigame {
  ...
  override async attach() {
    ...
    const texture = await Assets.load(MINIGAME_ASSET_ALIASES.BALANCE_BG);
  ...
  protected override async populateContainer() {
    ...
    switch (this.week) {
      case 0:
        ...
        this.pos = new Point(screen.width / 2, screen.height * 0.85);
      ...
      case 1:
        ...
        this.pos = new Point(screen.width / 2, (screen.height / 6) * 4.9);
      ...
      case 2:
        ...
        this.pos = new Point(screen.width / 2, (screen.height / 6) * 5.3);
        anchorPoint = new Point(0.5, 0.91);
        break;
    }

    this.balancingObject = new Sprite(await Assets.load(tex));
    const aspectRatio =
      this.balancingObject.width / this.balancingObject.height;
    this.balancingObject.height = screen.height * [0.5, 0.7, 0.9][this.week];
    this.balancingObject.width = this.balancingObject.height * aspectRatio;
  ...
  protected onUpdate(time: Ticker): void {
    ...
    if (this.balancingObject.rotation >= 0) {
      this.balancingObject.rotation +=
        this.gravityAmount +
        Math.abs(this.balancingObject.rotation) * this.randomFuckAssNumber;
    } else {
      this.balancingObject.rotation -=
        this.gravityAmount +
        Math.abs(this.balancingObject.rotation) * this.randomFuckAssNumber;
